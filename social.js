...
/* ============================================================
1. CONSTANTES DE STYLE (boutons & libellés réutilisés)
============================================================ */
const SOCIAL_STYLES = {
...
/* ============================================================
4. AMIS — ACTIONS (demande, acceptation, suppression)
============================================================ */
...
socket.on("receive_game_invite", (data) => {
  ...
  if (data.roomCode) {
    inviteHtml += `<br><button class="power-btn equip" onclick="joinGameInviteByCode('${data.roomCode}')" style="margin-top:6px; font-size:11px; padding:4px 10px;">${d.friend_join_room}</button>`;
  }
...
function renderGameInvitesList() {
  ...
  if (myGameInvites.length === 0) {
    container.innerHTML = `<div style="${SOCIAL_STYLES.emptyList}">${d.friend_no_invites}</div>`;
    return;
  }
...
/* ============================================================
6. AMIS — RENDU DE LA LISTE (socket)
============================================================ */
socket.on("friends_list_data", (friends) => {
  ...
  const makeLabel = (text) => {
    ...
    el.style.cssText = SOCIAL_STYLES.sectionLabel;
    el.innerText = text;
    return el;
  };
  ...
  const renderCard = (f) => {
    ...
    if (f.status === "pending") {
      if (!f.isRequester) {
        // Demande REÇUE : accepter ou refuser
        ...
        // Demande ENVOYÉE : en attente ou annuler
      ...
      if (f.isOnline && f.targetSocketId) {
        actionsHtml += `<button class="power-btn buy" onclick="inviteFriend('${f.targetSocketId}')" style="${SOCIAL_STYLES.smallBtn}">${d.friend_invite}</button>`;
      }
    }
    ...
    if (f.status === "accepted") {
      actionsHtml += `<button class="power-btn" onclick="removeFriend('${f.id}')" style="${SOCIAL_STYLES.removeBtn}">${d.friend_remove}</button>`;
    }
  ...
  if (currentFriendFilter === "requests") {
    if (incomingRequests.length === 0 && outgoingRequests.length === 0) {
      container.innerHTML = `<div style="${SOCIAL_STYLES.emptyList}">${d.friend_no_requests}</div>`;
      return;
    }
  ...
  if (accepted.length === 0) {
    container.innerHTML = `<div style="${SOCIAL_STYLES.emptyList}">${d.no_friends}</div>`;
    return;
  }
...
function joinRoomDirect(code, password) {
  socket.emit("join_room", { code: code.toUpperCase(), password });
}
...
async function shareRoomLink() {
  ...
  const d = i18n[currentLang];
  // 1) Feuille de partage native (WhatsApp, SMS, Mail, Copier…)
  if (navigator.share) {
    try {
      await navigator.share({ title: "Chiffre Blitz ⚡", text: d.share_text, url: url });
      return;
    } catch (e) { if (e && e.name === "AbortError") return; }
  }
  // 2) Fallback seulement si le partage natif est absent : presse-papier + toast
  try {
    await navigator.clipboard.writeText(url);
  } catch (e) {
    input.style.left = "0"; input.select(); document.execCommand("copy"); input.style.left = "-9999px";
  }
  showNotificationToast("📋 " + d.link_copied, "gift");
}

/* ============================================================
9. SALONS — HANDLERS SOCKET & RENDU
============================================================ */
socket.on("rooms_list_data", (rooms) => {
  ...
  if (!rooms || rooms.length === 0) {
    listEl.innerHTML = `<div style="text-align:center; color:#aaa; margin-top:8px; font-size:11px;">${d.no_rooms}</div>`;
    return;
  }
  rooms.forEach(r => {
    const row = document.createElement("div");
...
socket.on("rooms_list_changed", () => {
  if (document.getElementById("screen-rooms").style.display === "flex") fetchRoomsList();
});
...
function updateRoomPlayers(players) {
  ...
  if (!players || players.length === 0) {
    playersListEl.innerText = i18n[currentLang].waiting_opponent;
    return;
  }
...
/* ============================================================
11. REPRISE AUTO DU SALON DEPUIS L'URL (?room=CODE)
============================================================ */
