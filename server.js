...
const VERSION_GATE = {
  ...
  minShell: 12,           // versionCode mini accepté (en dessous = maj FORCÉE)
  latestShell: 13,       // dernier versionCode publié (en dessous = bandeau doux)
...
function maintAnnouncePayload(){
const remaining = Math.max(0, Math.round(((maintenanceState.kickAt || 0) - Date.now()) / 1000));
return { message: maintenanceState.message, delay: remaining, phase: (Date.now() >= (maintenanceState.warnAt || 0)) ? 'warn' : 'info' };
}
function armMaintenanceTimers(){
if (maintenanceWarnTimer) clearTimeout(maintenanceWarnTimer);
if (maintenanceKickTimer) clearTimeout(maintenanceKickTimer);
if (!maintenanceState.enabled) return;
const now = Date.now();
const warnIn = (maintenanceState.warnAt || 0) - now;
const kickIn = (maintenanceState.kickAt || 0) - now;
if (kickIn <= 0) { maintenanceKickAll(); return; }
if (warnIn > 0) {
maintenanceWarnTimer = setTimeout(() => {
for (const s of maintSockets()){ if (!isMaintBypass(s)) s.emit('maintenance_announce', maintAnnouncePayload()); }
}, warnIn);
}
maintenanceKickTimer = setTimeout(maintenanceKickAll, kickIn);
}
...
const delaySec = Math.max(0, Math.min(43200, parseInt(data && data.delay, 10) || 60));
if (want){
