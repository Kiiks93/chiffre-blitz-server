...
// ?dev=CODE → mémorise le code dev (une seule fois), puis recharge sans le paramètre
(function(){ const m = location.search.match(/[?&]dev=([^&]+)/); if (m) { localStorage.setItem('cb_web_dev_code', decodeURIComponent(m[1])); location.replace(location.pathname); } })();
...
function cbShowMobileOnly(){
  ...
  ov.innerHTML = '<div style="max-width:420px;width:100%;text-align:center;color:#fff;font-family:system-ui,sans-serif;">' +
    '<div style="font-size:52px;">📱</div>' +
    '<h2 style="color:#00d2ff;margin:12px 0 6px;font-size:22px;">CHIFFRE BLITZ SUR MOBILE</h2>' +
    '<p style="font-size:14px;line-height:1.5;color:#ddd;margin-bottom:18px;">Le jeu est disponible gratuitement sur Android.<br>(Passe de saison & packs dans l\'app.)</p>' +
