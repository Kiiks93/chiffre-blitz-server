...
const i18n = {
    fr: {
        ...
        powers: {
            ...
            quake: { name: '📳 Séisme', desc: 'Fait trembler la grille adverse (2s)' },
    ...
    en: {
        ...
        welcome_title: "⚡ BLITZ IDENTITY ⚡ ",
        welcome_sub: "Customize your competitive profile: ",
        pseudo_label: "PSEUDO ",
        avatar_num_label: "AVATAR (1-999) ",
        flag_label: "FLAG ",
        region_label: "REGION ",
...
// NOUVEAU : gestion des loadouts par partie
let currentMatchLoadout = [];
let currentMatchCharges = {};
let currentSoloLoadout = [];
let currentSoloCharges = {};

function countClientLoadout(loadout) {
    const counts = {};
...
function getOptionalLoadout() {
    const power = myProfile.equippedPower;

    if (power && (myProfile.inventory[power] || 0) > 0) {
        return [power];
    }
...
function updateEconomyUI() {
    ...
    if (titleEl) {
        titleEl.innerText = equippedTitle ? `[ ${TITLE_DISPLAY_NAMES[equippedTitle] || equippedTitle} ]` : "";
    }
...
function isProfileValid() {
    ...
    return savedName &&
        savedName.trim().length >= 3 &&
        savedName !== "Profil" &&
        savedName !== "Définir un pseudo" &&
        savedRegion;
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (targetRoom) {
        setTimeout(() => {
            if (isProfileValid()) {
                openJoinCustomScreen(targetRoom.toUpperCase());
            }
        }, 1000);
    }
});
    ...
    dash.innerHTML = `
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 15px;">
            <div class="roblox-card">
                <div style="font-weight: 900; color: #00d2ff; margin-bottom: 8px; font-size: 13px;">
                    📢 Diffuseur d'Annonce
                </div>
                <input type="text" id="admin-broadcast-text" placeholder="Message global..."
                    style="width: 100%; background: #0f051d; color: #fff; border: 2px solid #00d2ff; border-radius: 6px; padding: 8px; font-size: 11px; margin-bottom: 8px; box-sizing: border-box;">
                <button class="btn-main btn-blue" onclick="adminBroadcast()"
                    style="width: 100%; padding: 8px; font-size: 11px; margin-top: 0;">
                    Diffuser l'annonce 🚀
                </button>
            </div>

            <div class="roblox-card">
                <div style="font-weight: 900; color: #f8b500; margin-bottom: 8px; font-size: 13px;">
                    🎁 Distribution de Cadeaux Admin
                </div>

                <div style="display: flex; gap: 6px; margin-bottom: 8px;">
                    <select id="admin-target-type" onchange="toggleAdminTargetInput()"
                        style="flex: 1; background: #0f051d; color: #fff; border: 2px solid #f8b500; border-radius: 6px; padding: 6px; font-size: 11px;">
                        <option value="all">🌍 Tout le monde</option>
                        <option value="pseudo">👤 Par Pseudo</option>
                    </select>

                    <input type="text" id="admin-target-pseudo" placeholder="Pseudo exact..."
                        style="flex: 1; background: #0f051d; color: #fff; border: 2px solid #f8b500; border-radius: 6px; padding: 6px; font-size: 11px; display: none;">
                </div>
                ...
                <div style="display: flex; gap: 6px; margin-bottom: 8px;">
                    ...
                    <input type="number" id="admin-amount" placeholder="Montant" value="100"
                        style="flex: 1; background: #0f051d; color: #fff; border: 2px solid #f8b500; border-radius: 6px; padding: 6px; font-size: 11px;">
                </div>

                <button class="btn-main btn-gold" onclick="adminSendGift()"
                    style="width: 100%; padding: 8px; font-size: 11px; margin-top: 0;">
                    Envoyer le Cadeau ⚡
                </button>
            </div>
        ...
        <div class="roblox-card" style="flex: 1; display: flex; flex-direction: column;">
            <div style="font-weight: 900; color: #38ef7d; margin-bottom: 8px; font-size: 13px;">
                ⚡ Planification des Événements & Abuses
            </div>

            <div id="admin-schedules-container"
                style="flex: 1; overflow-y: auto; max-height: 250px; display: flex; flex-direction: column; gap: 6px; margin-bottom: 10px;">
            </div>

            <button class="btn-main btn-blue" onclick="saveAdminSchedules()"
                style="width: 100%; padding: 8px; font-size: 11px; margin-top: 0;">
...
function renderAdminSchedules(schedules) {
    ...
    for (let key in EVENT_NAMES) {
        ...
        row.innerHTML = `
            <div style="font-weight:bold; color:#fff; margin-bottom:4px; font-size:11px;">
                ${EVENT_NAMES[key]}
            </div>

            <div style="display:flex; align-items:center; gap:8px; margin-bottom:4px;">
                <label style="display:flex; align-items:center; gap:4px; cursor:pointer; font-size:10px; color:#aaa;">
                    <input type="checkbox" id="admin-manual-${key}" ${s.manual ? 'checked' : ''}>
                    Actif (Forcé)
                </label>
            </div>
            ...
            <div style="display:flex; flex-direction:column; gap:4px;">
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <input type="datetime-local" id="admin-start-${key}" value="${startStr}"
                        style="background:#0f051d; color:#fff; border:1px solid #444; border-radius:4px; padding:2px 4px; font-size:10px;">
                </div>
    ...
    myGameInvites.forEach((inv, index) => {
        ...
        row.innerHTML = `
            ...
            <div style="display:flex; gap:4px; align-items:center;">
                <button class="power-btn equip" onclick="closeFriendsModal(); joinRoomDirect('${inv.roomCode}', '')" style="font-size:10px; padding:4px 8px;">
                    Rejoindre ⚡
                </button>

                <button class="power-btn" onclick="removeGameInvite(${index})" style="font-size:10px; padding:4px 6px; background:rgba(255,75,43,0.2); color:#ff4b2b; border:1px solid #ff4b2b;">
                    ✕
                </button>
            </div>
        `;
...
socket.on('opponent_wants_rematch', () => {
    showNotificationToast("⚔️ L'adversaire souhaite une revanche !", 'gift');
});
...
function renderRankedLoadoutItems() {
    ...
    const ownedPowers = POWERS_CATALOG.filter(p =>
        p.type !== 'cosmetics' && (myProfile.inventory[p.id] || 0) > 0
    );

    if (ownedPowers.length === 0) {
        container.innerHTML = `
            <div style="grid-column: span 2; text-align:center; color:#aaa; padding:12px; font-size:11px;">
                Inventaire vide !
            </div>
        `;
        return;
    ...
    summary.innerHTML = `
        <b>Objets sélectionnés : ${selectedRankedItems.length}/2</b><br>
        ${selectedRankedItems.length === 2 ? "✅ Prêt à lancer" : "⚠️ Tu dois sélectionner exactement 2 objets"}
    `;
    ...
    ownedPowers.forEach(p => {
        ...
        const selectedCount = selectedRankedItems.filter(itemId => itemId === p.id).length;
        ...
        card.innerHTML = `
            ...
            <div style="font-weight:bold; font-size:10px; color:${selectedCount > 0 ? '#00ff88' : '#f8b500'};">
                Sélectionné : ${selectedCount}
            </div>

            <div style="display:flex; gap:4px; margin-top:6px;">
                <button class="power-btn buy" onclick="addRankedItem('${p.id}')"
                    ${(selectedRankedItems.length >= 2 || selectedCount >= qty) ? 'disabled' : ''}>
                    + Ajouter
                </button>

                <button class="power-btn" onclick="removeRankedItem('${p.id}')"
...
function addRankedItem(id) {
    ...
    const selectedCount = selectedRankedItems.filter(itemId => itemId === id).length;

    if (selectedCount >= owned) {
        alert("Tu ne possèdes pas assez d'exemplaires de cet objet.");
        return;
    }
...
function removeRankedItem(id) {
    const index = selectedRankedItems.lastIndexOf(id);

    if (index !== -1) {
        selectedRankedItems.splice(index, 1);
    }
...
function startRankedMatch() {
    if (selectedRankedItems.length !== 2) {
        alert('En mode classé, tu dois sélectionner exactement 2 objets.');
        return;
    }
    ...
    socket.emit('find_ranked_match', {
        items: selectedRankedItems.slice()
    });
}

function cancel1v1Search() {
    showMainMenu();
}
...
function copyRoomLink() {
    ...
    navigator.clipboard.writeText(input.value).then(() => {
        alert(i18n[currentLang].link_copied);
    });
}
    ...
    rooms.forEach(r => {
        ...
        row.innerHTML = `
            <span class="room-info">
                Salon <b>${r.code}</b>${lockIcon} (${r.playersCount}/2)
            </span>

            <button class="power-btn equip" onclick="joinRoomFromList('${r.code}', ${r.hasPassword})">
                Rejoindre
            </button>
        `;
...
function updateRoomPlayers(players) {
    ...
    if (!players || players.length === 0) {
        playersListEl.innerText = "En attente d'un adversaire...";
        return;
    }
    ...
    playersListEl.innerHTML = players.map(rawData => {
        ...
        const titleHtml = title
            ? `<span style="font-size: 8px; color: #f8b500; margin-left: 3px;">[${TITLE_DISPLAY_NAMES[title] || title}]</span>`
            : '';

        return `
            <div style="display:inline-flex; align-items:center; gap:4px;">
                ${getAvatarBadgeHTML(p.flag, p.avatar, null, p)}
    ...
    if (players && socket.id) {
        const opp = players.find(raw => (raw.socketId || raw.id) !== socket.id);
    ...
    if (!popup) {
        ...
        popup.innerHTML = `
            <div class="modal-card" style="text-align:center; animation: victoryScalePop 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards; border-color: #f8b500; box-shadow: 0 0 40px rgba(248,181,0,0.8);">
                ...
                <div style="font-size: 10px; font-weight: 900; color: #f8b500; letter-spacing: 2px; margin-bottom: 4px;">
                    RÉCOMPENSE DÉBLOQUÉE
                </div>

                <div id="popup-reward-name" style="color: #fff; font-size: 15px; font-weight: bold; margin-bottom: 20px; line-height: 1.4;">
                    -
                </div>

                <button class="btn-main btn-gold" onclick="document.getElementById('reward-popup-overlay').style.display='none'" style="width:100%; margin-top:0;">
                    Récupéré ! ⚡
                </button>
            </div>
        `;
...
function renderBlitzPass() {
    ...
    container.innerHTML = `
        <div class="bp-header-banner">
            <div style="font-size: 13px; font-weight: 900; color: #f8b500; margin-bottom: 2px;">
                🌟 SAISON 1 : PASSE DE COMBAT FÉLIN & NÉON
            </div>

            <div style="font-size: 10px; color: #ccc; margin-bottom: 6px;">
                ${isPremium ? '✨ Passe Premium Actif !' : 'Débloque le Passe Premium pour 1000 🪙'}
            </div>

            ${!isPremium
                ? `<button class="btn-main btn-gold" onclick="buyBlitzPassPremium()" style="padding: 6px 10px; font-size: 11px; margin: 0 auto; width: auto;">Acheter le Passe Premium (1000 🪙) ⭐</button>`
                : `<div style="color: #00ff88; font-weight: bold; font-size: 10px;">Statut : VIP / Premium</div>`
            }
        </div>
    `;
        ...
        card.innerHTML = `
            ...
            <div class="bp-tracks-grid">
                <div class="bp-track-box">
                    <div>
                        <span style="color:#38ef7d; font-weight:bold;">🟢 Gratuit :</span><br>
                        ${t.free}
                    </div>

                    <button class="power-btn ${isFreeClaimed ? 'active' : 'equip'}"
                        style="margin-top:4px; font-size:9px; padding:3px; ${isFreeClaimed ? 'opacity:0.5;' : ''}"
                        ${isFreeClaimed ? 'disabled' : ''}
                ...
                <div class="bp-track-box">
                    <div>
                        <span style="color:#00d2ff; font-weight:bold;">⭐ Premium :</span><br>
                        ${t.premium}
                    </div>

                    <button class="power-btn ${isPremClaimed ? 'active' : 'equip'}"
                        style="margin-top:4px; font-size:9px; padding:3px; ${isPremClaimed ? 'opacity:0.5;' : ''}"
                        ${isPremClaimed ? 'disabled' : ''}
...
function claimPassReward(tier, track) {
    ...
    const rewardText = tierData
        ? (track === 'premium' ? tierData.premium : tierData.free)
        : `Palier ${tier}`;

    const icon = (tier === 30 && track === 'premium')
        ? '🌈'
        : (tier === 15 && track === 'premium')
            ? '🐱'
            : '🌟';
...
function switchShopTab(type) {
    ...
    if (cosmeticsTabBtn) {
        cosmeticsTabBtn.classList.toggle('active', type === 'cosmetics');
    }
    ...
    const cosmeticsDict = {
        theme_glacial: {
            name: '🧊 Thème Cryo',
            desc: 'Grille aux reflets bleutés glacés'
        },
        frame_voltage: {
            name: '⚡ Cadre Sous Tension',
            desc: "Éclairs électriques crépitants autour de l'avatar"
        },
        frame_obsidian: {
            name: '🖤 Cadre Obsidienne',
            desc: 'Cadre sombre strié de lueurs pourpres'
        }
    };
    ...
    POWERS_CATALOG.filter(p => p.type === type).forEach(p => {
        ...
        if (type === 'cosmetics') {
            ...
            const equipped = myProfile.inventory &&
                myProfile.inventory.__equipped &&
            ...
            card.innerHTML = `
                ...
                <div style="font-weight:bold; margin-bottom:4px; font-size:10px; color:#f8b500;">
                    ${p.price} 🪙
                </div>

                ${unlocked
                    ? `<button class="power-btn ${equipped ? 'active' : 'equip'}" onclick="equipCosmetic('${p.id}')">${equipped ? 'Équipé ✅' : 'Équiper'}</button>`
...
function getAvatarBadgeHTML(flag, avatarNum, overrideAvatarType, playerObj) {
    ...
    const equippedAvatar = overrideAvatarType ||
        (profile.inventory && profile.inventory.__equipped && profile.inventory.__equipped.avatar);

    const equippedFrame = profile.inventory &&
        profile.inventory.__equipped &&
        profile.inventory.__equipped.frame;
    ...
    const html = `
        <div class="tft-avatar-container ${frameClass}" title="${avatarTitle}">
            <span class="tft-avatar-icon" style="display: flex; align-items: center; justify-content: center; width: 100%; height: 100%; ${typeof avatarContent === 'number' ? 'font-size: 14px;' : ''}">
                ${avatarContent}
            </span>
...
function getLargeAvatarBadgeHTML(flag, avatarNum, overrideAvatarType) {
    const avatarType = overrideAvatarType ||
        activeAvatarChoice ||
        (myProfile.inventory && myProfile.inventory.__equipped && myProfile.inventory.__equipped.avatar);

    const equippedFrame = myProfile.inventory &&
        myProfile.inventory.__equipped &&
        myProfile.inventory.__equipped.frame;
    ...
    const html = `
        <div class="tft-avatar-large ${frameClass}">
            <span class="tft-avatar-large-icon" style="display: flex; align-items: center; justify-content: center; width: 100%; height: 100%; ${typeof avatarContent === 'number' ? 'font-size: 24px;' : ''}">
                ${avatarContent}
            </span>
...
function renderProfileAvatarSelector() {
    ...
    const addAvatarOption = (id, icon, label) => {
        ...
        card.style.cssText = `
            flex: 1;
            min-width: 80px;
            background: ${isActive ? 'rgba(0,210,255,0.25)' : 'rgba(255,255,255,0.05)'};
        ...
        card.innerHTML = `
            ...
            <div style="font-size: 8px; font-weight: bold; color: #fff;">${label}</div>
        `;
    ...
    if (unlocked.includes('avatar_lottie_palier15')) {
        addAvatarOption('avatar_lottie_palier15', '🐱', 'Chat Assistant');
    }

    if (unlocked.includes('avatar_lottie_palier30')) {
        addAvatarOption('avatar_lottie_palier30', '🌈', 'Chat Arc-en-ciel');
    }
}
...
function renderProfileCustomizationMenus() {
    ...
    const equippedTitle = myProfile.inventory &&
        myProfile.inventory.__equipped &&
        myProfile.inventory.__equipped.title;
    ...
    if (titleSelect) {
        ...
        unlockedTitles.forEach(tId => {
            ...
            if (equippedTitle === tId || equippedTitle === displayName) {
                opt.selected = true;
            }
    ...
    const equippedTheme = myProfile.inventory &&
        myProfile.inventory.__equipped &&
        myProfile.inventory.__equipped.theme;
...
function preparePowerHUD() {
    ...
    let loadout = isSolo ? currentSoloLoadout : currentMatchLoadout;
    let charges = isSolo ? currentSoloCharges : currentMatchCharges;

    // Sécurité si le serveur n'a pas encore envoyé le loadout
    if (!loadout || loadout.length === 0) {
        if (myProfile.equippedPowers && myProfile.equippedPowers.length > 0) {
            loadout = myProfile.equippedPowers.slice();
        } else if (myProfile.equippedPower) {
            loadout = [myProfile.equippedPower];
        } else {
            loadout = [];
        }

        charges = countClientLoadout(loadout);
    }

    let usableCount = 0;

    loadout.forEach(powerId => {
        let remaining = charges[powerId] || 0;

        // Sécurité de secours basée sur l'inventaire
        if (remaining <= 0) {
...
function triggerSpecificPower(powerId, btnEl) {
    ...
    if (!charges[powerId] || charges[powerId] <= 0) {
        const stock = myProfile.inventory[powerId] || 0;
    ...
    // sont maintenant envoyés automatiquement par le serveur
    // via use_power. On ne fait plus socket.emit('send_malus').
...
socket.on('solo_session_started', (data) => {
    currentSoloLoadout = data.loadout || [];
    currentSoloCharges = data.remainingCharges || {};
...
function fetchLeaderboard() {
    ...
    document.getElementById('lb-list').innerHTML = `
        <div style="text-align:center; color:#aaa; margin-top:15px; font-size:11px;" data-i18n="loading">
            Chargement...
        </div>
    `;
...
socket.on('leaderboard_data', (res) => {
    ...
    if (!res.data || res.data.length === 0) {
        container.innerHTML = `
            <div style="text-align:center; color:#aaa; margin-top:15px; font-size:11px;">
                Aucun joueur.
            </div>
        `;
        return;
    ...
    parsedList.forEach((p, index) => {
        ...
        const equippedTitle = p.inventory &&
            p.inventory.__equipped &&
            p.inventory.__equipped.title;

        const titleHtml = equippedTitle
            ? `<span style="font-size: 8px; color: #f8b500; font-weight: bold; margin-left: 4px;">[${TITLE_DISPLAY_NAMES[equippedTitle] || equippedTitle}]</span>`
            : '';

        let rightBadge = `<span class="lb-pts" style="color:#00ff88;">${p.points} pts</span>`;

        if (category === 'coins') {
            rightBadge = `<span class="lb-pts" style="color:#f8b500;">${p.coins} 🪙</span>`;
        } else if (category === 'trophies') {
            rightBadge = `<span class="lb-pts" style="color:#fceabb;">${p.trophies} 🏆</span>`;
        } else if (category === 'combined') {
            rightBadge = `<span class="lb-pts" style="color:#00d2ff; font-size:11px;">🏆${p.trophies} | ${p.points}pts</span>`;
        }
...
/* ============================================================
   AFFICHAGE ADVERSAIRE / MATCH 1V1
============================================================ */
...
function updateOpponentDisplay(opp) {
    ...
    const oppTitle = cachedOpponent.inventory &&
        cachedOpponent.inventory.__equipped &&
        cachedOpponent.inventory.__equipped.title;

    const oppTitleEl = document.getElementById('opp-profile-title');

    if (oppTitleEl) {
        oppTitleEl.innerText = oppTitle
            ? `[ ${TITLE_DISPLAY_NAMES[oppTitle] || oppTitle} ]`
            : "";
    }
}
...
socket.on('start_countdown', (data) => {
    ...
    currentMatchLoadout = data.myLoadout || [];
    currentMatchCharges = data.myRemainingCharges || {};

    // Sécurité si le serveur n'envoie pas encore le loadout
    if (!currentMatchLoadout || currentMatchLoadout.length === 0) {
        if (myProfile.equippedPowers && myProfile.equippedPowers.length > 0) {
            currentMatchLoadout = myProfile.equippedPowers.slice();
        } else if (myProfile.equippedPower) {
            currentMatchLoadout = [myProfile.equippedPower];
        } else {
            currentMatchLoadout = [];
        }

        currentMatchCharges = countClientLoadout(currentMatchLoadout);
    }

    let oppData = extractOpponentInfo(data);

    if (oppData) {
        updateOpponentDisplay(oppData);
    }
...
socket.on('jackpot_wheel_result', (data) => {
    ...
    setTimeout(() => {
        if (data.outcome === 'jackpot') {
            ...
            const powerInfo = i18n[currentLang].powers[data.itemId];
            const itemName = powerInfo ? powerInfo.name : '';

            resultText.innerHTML = `🎁 <span style="color:#00c6ff;">OBJET GAGNÉ ! ${itemName}</span>`;
            SoundEngine.playVictory();
...
function getWinnerAvatarShowcaseHTML(playerObj) {
    ...
    const equippedAvatar = playerObj.inventory &&
        playerObj.inventory.__equipped &&
        playerObj.inventory.__equipped.avatar;

    const equippedFrame = playerObj.inventory &&
        playerObj.inventory.__equipped &&
        playerObj.inventory.__equipped.frame;
    ...
    const html = `
        <div class="victory-avatar-showcase">
            <div class="victory-badge-large ${frameClass}" style="display: flex; align-items: center; justify-content: center;">
                ...
                <span style="position: absolute; bottom: -2px; right: -2px; font-size: 14px; background: #0f051d; border-radius: 50%; width: 22px; height: 22px; display: flex; align-items: center; justify-content: center; border: 2px solid #fff; z-index:3;">
                    ${playerObj.flag || '🇫🇷'}
                </span>
            </div>

            <div style="font-size: 13px; font-weight: 900; color: #f8b500; margin-top: 4px;">
                ${playerObj.username || 'Joueur'} TRIOMPHE !
            </div>
        </div>
    `;
...
function showGameOverRecap(data) {
    ...
    if (rematchBtn) {
        ...
        rematchBtn.innerText = "Revanche ⚔️";
    }

    const myReward = data.rewards && data.rewards[myId]
        ? data.rewards[myId]
        : { baseCoins: 30, rushBonus: 0, totalCoins: 30 };
    ...
    if (modalCard) {
        ...
        if (!isWinner && winnerId) {
            modalCard.classList.add('defeat-theme');
        }
    }
    ...
    if (winnerObj) {
        ...
        cinematicContainer.innerHTML = `
            <div class="victory-avatar-showcase">
                ...
                <div style="font-size: 13px; font-weight: 900; color: #00d2ff;">ÉGALITÉ !</div>
            </div>
        `;
    }

    if (isWinner) {
        banner.innerText = "🏆 VICTOIRE SUPRÊME !";
        banner.style.color = "#00ff88";
        SoundEngine.playVictory();
    } else if (winnerId) {
        banner.innerText = "💥 DÉFAITE AMÈRE...";
        banner.style.color = "#ff4b2b";
    } else {
        banner.innerText = "⏱️ ÉGALITÉ !";
    ...
    if (myReward.rushBonus > 0) {
        htmlCoins += ` <span style="color:#ff8a00;">+${myReward.rushBonus}(RUSH)</span>`;
    }
...
function startSoloTraining(mode) {
    ...
    soloTarget = (activeTrainingMode === 'random')
    ...
    const soloLoadout = getOptionalLoadout();

    currentSoloLoadout = soloLoadout;
    currentSoloCharges = countClientLoadout(soloLoadout);
...
function handleAvalancheClick(val, idx) {
    ...
    if (tiles[idx]) {
        ...
        setTimeout(() => {
            tiles[idx].classList.remove('ripple-active');
        }, 400);
    }
