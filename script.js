...
const SoundEngine = {
    ...
    // 🥁 Kick doux (juste pour le mouvement)
    if (inBar % 4 === 0) {
    ...
    // 🎸 Basse ronde (début de mesure) + respiration (milieu)
    if (inBar === 0) {
        const osc = this.ctx.createOscillator(), gain = this.ctx.createGain();
        osc.type = 'sine';
    ...
    // 🌫️ Nappe d'accords (attaque lente, très douce)
    if (inBar === 0) {
    ...
    // 🎼 MÉLODIE fluide et legato (le cœur du morceau)
    if (inBar % 2 === 0) {
        const melodyA = [
        ...
        if (note > 0) {
            // Voix principale : triangle, enveloppe legato (les notes se chevauchent)
            const osc = this.ctx.createOscillator(), gain = this.ctx.createGain();
            osc.type = 'triangle';
            ...
            // Doublure scintillante une octave plus haut (très discrète)
            const o2 = this.ctx.createOscillator(), g2 = this.ctx.createGain();
            o2.type = 'sine';
            o2.frequency.setValueAtTime(note * 2, t);
            g2.gain.setValueAtTime(0.0001, t);
            g2.gain.linearRampToValueAtTime(0.012, t + 0.03);
...
function updateEconomyUI() {
    ...
    if (titleEl) {
        titleEl.innerText = equippedTitle
            ? `[ ${TITLE_DISPLAY_NAMES[equippedTitle] || equippedTitle} ]`
            : "";
    }
...
function updateShopCoinsDisplay() {
    ...
    if (valEl) {
        valEl.innerText = myProfile.coins;
    }
}
...
socket.on("player_registered", (rawData) => {
    ...
    const localEquipped = myProfile.inventory && myProfile.inventory.__equipped
        ? { ...myProfile.inventory.__equipped }
        : {};
...
socket.on("admin_auth_fail", (msg) => {
    ...
    if (errEl) {
        errEl.innerText = msg || "Mot de passe incorrect";
    }
});
...
function updateFriendsBadge() {
    ...
    if (requestsTab) {
        requestsTab.innerText = `Demandes ${window.lastRequestsCount || 0}`;
    }

    if (invitesTab) {
        invitesTab.innerText = `Invitations ${myGameInvites.length}`;
    }
}
...
function switchFriendTab(tab) {
    ...
    if (invitesTab) {
        invitesTab.classList.toggle("active", tab === "invites");
    }
...
socket.on("receive_game_invite", (data) => {
    ...
    if (data.roomCode) {
        inviteHtml += `<br><button class="power-btn equip" onclick="joinGameInviteByCode('${data.roomCode}')" style="margin-top:6px; font-size:11px; padding:4px 10px;">Rejoindre le salon ⚡</button>`;
    }
    ...
    myGameInvites.forEach((inv, index) => {
        ...
        row.innerHTML = `
            ...
            <div style="display:flex; gap:4px; align-items:center;">
                <button class="power-btn equip" onclick="joinGameInviteByCode('${inv.roomCode}')" style="font-size:10px; padding:4px 8px;">
                    Rejoindre ⚡
                </button>
...
function copyRoomLink() {
    ...
    navigator.clipboard.writeText(input.value).then(() => {
        showNotificationToast("📋 " + i18n[currentLang].link_copied, "gift");
    });
}
...
socket.on("rooms_list_changed", () => {
    if (document.getElementById("screen-rooms").style.display === "flex") {
        fetchRoomsList();
    }
});
            ...
            <div class="bp-tracks-grid">
                ...
                <div class="bp-track-box">
                    ...
                    <button class="power-btn ${isPremClaimed ? "active" : "equip"}"
                        style="margin-top:4px; font-size:9px; padding:3px; ${premDisabled ? "opacity:0.5;" : ""}"
