...
function buildModeWheel() {
  ...
  visibleWheelModes().forEach(function(m){
    ...
    b.onclick = function(){
      const i = visibleWheelModes().indexOf(m);
      if (i === Math.round(wheelCurrent)) launchMode(m);
      else { wheelCurrent = i; renderWheel(); }
    };
    wrap.appendChild(b);
  });
  renderWheel();
...
(function(){
  ...
  document.addEventListener('touchstart', function(e){
    ...
    if (!z) return;
    drag = true; sx = e.touches[0].clientX; sc = wheelCurrent;
  }, { passive: true });
  document.addEventListener('touchmove', function(e){
    if (!drag) return;
    const dx = e.touches[0].clientX - sx;
    wheelCurrent = sc - dx / 70;
...
function syncMobileMenuButtons() {
  [['btn-halloween-menu','mobile-btn-halloween'],
   ...
   ['btn-tow-menu','mobile-btn-tow']].forEach(function(p){
...
(function(){
  ...
  HIDE.forEach(function(fn){
    const w = setInterval(function(){
      if (typeof window[fn] === 'function' && !window[fn].__h) {
        const o = window[fn];
        window[fn] = function(){ showMobileMenu(false); return o.apply(this, arguments); };
        window[fn].__h = true;
        clearInterval(w);
      }
    }, 100);
  ...
  SHOW.forEach(function(fn){
    const w = setInterval(function(){
      if (typeof window[fn] === 'function' && !window[fn].__s) {
        const o = window[fn];
        window[fn] = function(){ const r = o.apply(this, arguments); showMobileMenu(true); return r; };
        window[fn].__s = true;
        clearInterval(w);
      }
    }, 100);
...
document.addEventListener('DOMContentLoaded', function(){
  ['btn-halloween-menu','btn-noel-menu','btn-tow-menu'].forEach(function(id){
